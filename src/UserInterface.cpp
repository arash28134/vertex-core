...
// INITIALIZATION
//================
void Engine::Window::UserInterface::initializeGraphics()
{
	// Took from "OpenGLTemplate", Advaned Rendering, MSc Computer Graphics, Virtual Reality, and Videogames
...
void Engine::Window::UserInterface::createFontsTexture()
{
...
void Engine::Window::UserInterface::updateGraphics()
{
...
void Engine::Window::UserInterface::render(double deltaTime)
{
	...
	glfwGetWindowSize(surface, &w, &h);
	glfwGetFramebufferSize(surface, &display_w, &display_h);

	io.DisplaySize = ImVec2((float)w, (float)h);
	io.DisplayFramebufferScale = ImVec2(w > 0 ? ((float)display_w / w) : 0, h > 0 ? ((float)display_h / h) : 0);

	// Setup time step
	io.DeltaTime = deltaTime > 0.0 ? float(deltaTime) : (float)(1.0f / 60.0f);

	// Setup inputs
	// (we already got mouse wheel, keyboard keys & characters from glfw callbacks polled in glfwPollEvents())
	if (glfwGetWindowAttrib(surface, GLFW_FOCUSED))
	{
		double mouse_x, mouse_y;
		glfwGetCursorPos(surface, &mouse_x, &mouse_y);
	...
	for (int i = 0; i < 3; i++)
	{
		...
		io.MouseDown[i] = g_MousePressed[i] || glfwGetMouseButton(surface, i) != 0;    
...
void Engine::Window::UserInterface::release()
{
	// Release allocated resources on GPU
	if (g_VaoHandle) 
...
void Engine::Window::UserInterface::updateMouseButtonPressed(int button, bool pressed)
{
